...
const mockSteps: Step[] = [
  {
    ...
    description: 'Set up the basic project structure with React and necessary dependencies.',
    status: 'completed'
  },
  {
    id: 2,
    title: 'Create Components',
    description: 'Build the core components for the application based on the requirements.',
    status: 'in-progress'
  },
  {
    id: 3,
...
export const usePromptStore = create<PromptState>((set) => ({
  ...
  addFile: (parentPath: string, newFile: Omit<FileNode, 'id'>) => set((state) => {
  ...
  completeStep: (id: number) => set((state) => ({
    steps: state.steps.map(step => 
      step.id === id ? { ...step, status: 'completed' } : step
    ),
    currentStepId: id < Math.max(...state.steps.map(s => s.id)) ? id + 1 : id
  })),
